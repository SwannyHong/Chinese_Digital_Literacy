...
import dataBtnImg1 from './file1.png';
import dataBtnImg2 from './file2.png';
  ...
  return (
    <div className="min-h-screen bg-white w-full flex flex-col md:flex-row p-10 xl:px-20 overflow-hidden box-border relative">
      <motion.div className="absolute top-10 left-10 md:left-20 cursor-pointer group z-50" onClick={onBack} initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: 0.2 }}>
        <h2 className="text-4xl md:text-5xl font-serif font-black text-slate-900 group-hover:opacity-70 transition-opacity">The Next is YOU.</h2>
          ...
          <motion.div key="input" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, x: -50 }} className="w-full flex flex-col md:flex-row h-full pt-20">
            <div className="flex-1 flex flex-col justify-center h-full px-4 md:px-10 lg:px-20">
              <div className="w-full max-w-lg mx-auto">
                <h3 className="text-2xl md:text-3xl font-dream text-slate-900 mb-12">
                  직접 체험해보세요
                </h3>
...
function DataPage({ onBack }) {
  ...
  const menuItems = [
    ...
    { 
      ...
      alt: "라이브커머스 플랫폼", 
      img: dataBtnImg2, 
      link: "https://sites.google.com/khu.ac.kr/wanghong-agency/analytics/%EB%9D%BC%EC%9D%B4%EB%B8%8C%EC%BB%A4%EB%A8%B8%EC%8A%A4-%ED%94%8C%EB%9E%AB%ED%8F%BC" 
    }
  ];
        ...
        <motion.div className="w-full max-w-[700px] grid grid-cols-1 md:grid-cols-2 gap-8 xl:gap-12" variants={containerVariants} initial="hidden" animate="visible">
          {menuItems.map((item) => (
            <motion.button key={item.id} variants={itemVariants} whileHover={{ y: -12, scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleNavigate(item.link)} className="relative w-full aspect-square group cursor-pointer transition-all duration-300 bg-transparent outline-none">
              ...
              <div className="absolute inset-0 bg-white/0 group-hover:bg-white/10 transition-colors duration-300 rounded-[32px]" />
            </motion.button>
          ))}
        </motion.div>
      </div>
        ...
        {stage === 2 && (
          <motion.div className="w-full flex flex-col z-30 relative" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
            <div className="h-screen w-full flex flex-col items-start justify-center pl-10 md:pl-32 relative">
              ...
              <motion.p custom={2} variants={textVariants} initial="hidden" animate="visible" className="mt-12 text-xl md:text-2xl font-dream text-slate-500">
                당신의 영향력을 체험해보세요
              </motion.p>
